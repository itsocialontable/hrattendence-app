...
class _AdminDashboardState extends State<AdminDashboard> {
        ...
        child: Container(
          ...
          child: BottomNavigationBar(
            ...
            items: const [
              ...
              BottomNavigationBarItem(icon: Icon(Icons.bar_chart_outlined), activeIcon: Icon(Icons.bar_chart), label: 'Attendence'),
              BottomNavigationBarItem(icon: Icon(Icons.calculate_outlined), activeIcon: Icon(Icons.calculate), label: 'Salary'),
            ],
          ),
        ),
      ),
  ...
  Widget _buildHome(dynamic user) {
    ...
    final statItems = [
      _StatItem('Total Employees', stats?.totalEmployees.toString() ?? '—', Icons.people_alt_outlined, AppColors.secondary, AppColors.secondaryBg),
      _StatItem('Present Today',   stats?.presentToday.toString()   ?? '—', Icons.check_circle_outline, AppColors.success,  AppColors.successBg),
      _StatItem('On Leave',        stats?.onLeave.toString()        ?? '—', Icons.event_busy_outlined,  AppColors.warning,  AppColors.warningBg),
      _StatItem('Absent',          stats?.absent.toString()         ?? '—', Icons.cancel_outlined,      AppColors.error,    AppColors.errorBg),
    ];
    ...
    return RefreshIndicator(
      ...
      child: CustomScrollView(
        slivers: [
              ...
              child: Column(children: [
                Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Admin Panel', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 12)),
                ...
                Container(
                  ...
                  child: Row(children: [
                    ...
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text(_todayDate(), style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
                    ]),
                    // const Spacer(),
                    // Container(
                    //   padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                    //   decoration: BoxDecoration(color: AppColors.success, borderRadius: BorderRadius.circular(20)),
                    //   child: const Text('Working Day', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w500)),
                    // ),
                  ]),
                ),
              ]),
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Text(dp.error!, style: const TextStyle(color: AppColors.error, fontSize: 12))),
                    TextButton(
                      onPressed: () => context.read<AdminDashboardProvider>().fetchStats(),
          ...
          SliverPadding(
            ...
            sliver: SliverToBoxAdapter(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Overview', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.textDark)),
                SizedBox(height: 12,),
                dp.isLoading
                    ? const Padding(
          ...
          // Today attendance — live data if available
          SliverPadding(
            padding: const EdgeInsets.fromLTRB(20, 20, 20, 100),
            sliver: SliverToBoxAdapter(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  const Text("Today's Attendance", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.textDark)),
                  TextButton(
...
// Live attendance summary widget using real API data
class _LiveAttendanceSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: AppShadow.card),
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(children: [
          _SummaryRow('Present', stats.presentToday.toString(), AppColors.success),
          const Divider(height: 16, color: AppColors.border),
          _SummaryRow('On Leave', stats.onLeave.toString(), AppColors.warning),
          const Divider(height: 16, color: AppColors.border),
          _SummaryRow('Absent', stats.absent.toString(), AppColors.error),
          if (stats.lateToday != null && stats.lateToday > 0) ...[
            const Divider(height: 16, color: AppColors.border),
            _SummaryRow('Late', stats.lateToday.toString(), AppColors.warning),
          ],
        ]),
      ),
    );
...
class _SummaryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    const SizedBox(width: 10),
    Text(label, style: const TextStyle(fontSize: 14, color: AppColors.textMid)),
    const Spacer(),
...
class _PlaceholderAttendance extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: const Center(child: Text('Pull to refresh to load live data', style: TextStyle(color: AppColors.textLight, fontSize: 13))),
  );
}

// ── Helper models & widgets (exact same as original) ──────────────────────────
...
class _StatCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: stat.bg, borderRadius: BorderRadius.circular(10)),
          child: Icon(stat.icon, color: stat.color, size: 18)),
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(stat.value, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: stat.color)),
        Text(stat.label, style: const TextStyle(fontSize: 11, color: AppColors.textLight)),
      ]),
    ]),
  );
}
...
class _QuickActionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: AppShadow.card),
      child: Column(children: [
