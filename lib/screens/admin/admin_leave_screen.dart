...
class _AdminLeaveScreenState extends State<AdminLeaveScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AdminLeaveProvider>().fetchLeaves();
    });
  }
  ...
  void _openDetail(AdminLeaveRequest leave) async {
    if (leave.status != 'pending') {
      _showDetailOnly(leave);
      return;
    }
    final result = await showModalBottomSheet<_LeaveAction>(
      context: context, isScrollControlled: true, backgroundColor: Colors.transparent,
      builder: (_) => _LeaveDetailSheet(leave: leave),
    );
    if (result == null) return;
    final prov = context.read<AdminLeaveProvider>();
    bool ok;
    if (result.approve) {
      ok = await prov.approveLeave(leave.id, remark: result.remark, approvedBy: 'Admin');
    } else {
      ok = await prov.rejectLeave(leave.id, remark: result.remark, approvedBy: 'Admin');
    }
    if (mounted) {
      _showSnack(
        ok ? (result.approve ? '✓ Leave approved for ${leave.employeeName}' : '✗ Leave rejected for ${leave.employeeName}')
           : prov.error ?? 'Operation failed',
        ok ? (result.approve ? AppColors.success : AppColors.error) : AppColors.textDark,
      );
      if (ok) _tabCtrl.animateTo(result.approve ? 1 : 2);
    }
  }

  void _showDetailOnly(AdminLeaveRequest leave) {
    showModalBottomSheet(
      context: context, isScrollControlled: true, backgroundColor: Colors.transparent,
      builder: (_) => _LeaveDetailSheet(leave: leave, readOnly: true),
    );
  }
  ...
  Widget build(BuildContext context) {
    final prov = context.watch<AdminLeaveProvider>();
    final pending  = prov.leaves.where((l) => l.status == 'pending').toList();
    final approved = prov.leaves.where((l) => l.status == 'approved').toList();
    final rejected = prov.leaves.where((l) => l.status == 'rejected').toList();
      ...
      body: prov.isLoading
          ...
          : prov.hasError
              ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Text(prov.error!, style: GoogleFonts.poppins(color: AppColors.textMid, fontSize: 13)),
                  const SizedBox(height: 16),
                  ElevatedButton(
                    onPressed: () => context.read<AdminLeaveProvider>().fetchLeaves(),
              ...
              : NestedScrollView(
                  headerSliverBuilder: (_, __) => [
                    SliverAppBar(
                      ...
                      backgroundColor: AppColors.secondary, foregroundColor: AppColors.white, elevation: 0,
                      flexibleSpace: FlexibleSpaceBar(
                        background: Container(
                          ...
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                            ...
                            Text('Leave Requests', style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.white)),
                                SizedBox(height: 40,),
                          ]),
                        ),
                      ),
                      bottom: TabBar(
                        controller: _tabCtrl,
                        indicatorColor: AppColors.white, indicatorWeight: 3,
...
class _LeaveList extends StatelessWidget {
  final List<AdminLeaveRequest> leaves;
  final ValueChanged<AdminLeaveRequest> onTap;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: AppShadow.subtle, border: Border.all(color: _statusColor.withOpacity(0.15)),
        ),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(
            ...
            child: Row(children: [
              Container(
                ...
                child: Center(child: Text(initials, style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.white))),
              ),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(leave.employeeName, style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
                Text('${leave.userId} · ${leave.department}', style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textLight)),
              ])),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(color: _statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                child: Text(_statusLabel, style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w600, color: _statusColor)),
              ),
            ]),
          ),
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              Text(leave.leaveType, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.secondary)),
              const Spacer(),
              Icon(Icons.calendar_month_outlined, size: 13, color: AppColors.textLight),
              const SizedBox(width: 4),
              Text('${leave.fromDate} → ${leave.toDate}', style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textMid)),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                decoration: BoxDecoration(color: AppColors.primaryBg, borderRadius: BorderRadius.circular(6)),
                child: Text('${leave.totalDays}d', style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.primary)),
              ),
            ]),
          ),
          Padding(
            padding: const EdgeInsets.fromLTRB(16, 0, 16, 12),
            child: Text(leave.reason, maxLines: 2, overflow: TextOverflow.ellipsis,
                style: GoogleFonts.poppins(fontSize: 12, color: AppColors.textMid, height: 1.4)),
          ),
          Container(
            padding: const EdgeInsets.fromLTRB(16, 10, 16, 12),
            decoration: BoxDecoration(color: AppColors.background, borderRadius: const BorderRadius.vertical(bottom: Radius.circular(18))),
            child: Row(children: [
              Text('Applied: ${leave.appliedOn}', style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textLight)),
              const Spacer(),
              if (showActions) ...[
                ...
                Text('Tap to view details', style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textLight, decoration: TextDecoration.underline)),
            ]),
          ),
        ]),
      ),
...
class _LeaveAction { final bool approve; final String? remark; _LeaveAction({required this.approve, this.remark}); }

class _LeaveDetailSheet extends StatefulWidget {
  final AdminLeaveRequest leave;
  final bool readOnly;
  const _LeaveDetailSheet({required this.leave, this.readOnly = false});
  @override
...
class _LeaveDetailSheetState extends State<_LeaveDetailSheet> {
  ...
  bool get _isPending => widget.leave.status == 'pending' && !widget.readOnly;

  @override
  void initState() { super.initState(); _remarkCtrl.text = widget.leave.managerRemark ?? ''; }
  @override
  void dispose() { _remarkCtrl.dispose(); super.dispose(); }

  void _action(bool approve) async {
    setState(() => _submitting = true);
    await Future.delayed(const Duration(milliseconds: 300));
    if (mounted) Navigator.pop(context, _LeaveAction(approve: approve,
        remark: _remarkCtrl.text.trim().isEmpty ? (approve ? 'Approved by manager.' : 'Rejected by manager.') : _remarkCtrl.text.trim()));
  }

  @override
  Widget build(BuildContext context) {
    final l = widget.leave;
    final statusColor = l.status == 'pending' ? AppColors.warning : l.status == 'approved' ? AppColors.success : AppColors.error;
    final statusLabel = l.status == 'pending' ? 'Pending' : l.status == 'approved' ? 'Approved' : 'Rejected';
      ...
      builder: (_, ctrl) => Container(
        ...
        child: Column(children: [
          ...
          Container(
            ...
            decoration: const BoxDecoration(gradient: AppColors.secondaryGradient, borderRadius: BorderRadius.vertical(top: Radius.circular(28))),
            child: Row(children: [
              Container(
                width: 52, height: 52,
                decoration: BoxDecoration(color: AppColors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(15)),
                child: Center(child: Text(l.employeeName.split(' ').map((e) => e.isNotEmpty ? e[0] : '').take(2).join(),
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('${l.userId} · ${l.department}', style: GoogleFonts.poppins(fontSize: 12, color: AppColors.white.withOpacity(0.75))),
              ])),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _DetailRow(icon: Icons.info_outline_rounded, label: 'Status',
                    valueWidget: Container(
                      ...
                      child: Text(statusLabel, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600, color: statusColor)),
                    )),
                ...
                const SizedBox(height: 8),
                TextFormField(
                  controller: _remarkCtrl, maxLines: 3, readOnly: !_isPending,
                  ...
                  decoration: InputDecoration(
                    ...
                    filled: true, fillColor: _isPending ? AppColors.background : AppColors.neutralGreyLight.withOpacity(0.3),
          ...
          if (_isPending)
                  ...
                  : Row(children: [
                      Expanded(child: OutlinedButton.icon(
                        ...
                        style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14),
                            foregroundColor: AppColors.error, side: const BorderSide(color: AppColors.error, width: 1.5),
            ...
            Padding(
              padding: const EdgeInsets.fromLTRB(20, 12, 20, 28),
              child: Text(
                l.status == 'approved' ? '✓ Approved by ${l.approvedBy ?? "Admin"}' : '✗ Rejected by ${l.approvedBy ?? "Admin"}',
                textAlign: TextAlign.center,
                style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w500,
                    color: l.status == 'approved' ? AppColors.success : AppColors.error),
              ),
            ),
        ]),
      ),
...
class _DetailRow extends StatelessWidget {
  final IconData icon; final String label, value; final Widget? valueWidget;
  const _DetailRow({required this.icon, required this.label, this.value = '', this.valueWidget});
  @override
  Widget build(BuildContext context) => Padding(
    padding: const EdgeInsets.only(bottom: 12),
    child: Row(children: [
      Icon(icon, size: 16, color: AppColors.textLight), const SizedBox(width: 10),
