...
class ApiService {
  static const String baseUrl = 'https://attendx-backend-crei.onrender.com';
  //  static const String baseUrl = 'https://whacking-dispute-agility.ngrok-free.dev';
  ...
  Future<LoginResponse> employeeLogin({
    ...
    try {
      response = await HttpLogger.request(
        'POST',
        Uri.parse('$baseUrl/api/employee/login'),
        headers: {
  ...
  /// Get stored token
  /// Wrapped in try-catch because Android's encrypted storage keystore can
  /// become invalid after app reinstall/signing-key change, throwing
  /// javax.crypto.BadPaddingException. We treat that as "no token" instead
  /// of crashing the app on startup.
  Future<String?> getToken() async {
    try {
      return await _secureStorage.read(key: _tokenKey);
    } catch (e) {
      debugPrint('SecureStorage read failed (getToken): $e — clearing corrupted storage');
  ...
  Future<UserData?> getStoredUser() async {
    try {
      ...
      debugPrint('SecureStorage read failed (getStoredUser): $e — clearing corrupted storage');
      await _clearCorruptedStorage();
      return null;
    }
  }

  /// Wipes secure storage when it's unreadable (corrupted keystore).
  /// This forces the user back to the login screen instead of crashing.
  Future<void> _clearCorruptedStorage() async {
    try {
      await _secureStorage.deleteAll();
    } catch (_) {
      // If even deleteAll fails, there's nothing more we can do —
      // swallow it so the app can still boot.
    }
  }
