  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        // ── Admin API Service (depends on ApiService) ──
        Provider<AdminApiService>(
          create: (ctx) => AdminApiService(ctx.read<ApiService>()),
        ),

        // ── Admin Providers (auth, dashboard, employees, attendance, leave, salary, settings) ──
        // Registered at top-level so login/forgot-password/register/OTP screens
        // (which run BEFORE admin login) can also access AdminAuthProvider.
        ChangeNotifierProvider<AdminAuthProvider>(
          create: (ctx) => AdminAuthProvider(ctx.read<AdminApiService>()),
        ),
        ChangeNotifierProvider<AdminDashboardProvider>(
          create: (ctx) => AdminDashboardProvider(ctx.read<AdminApiService>()),
        ),
        ChangeNotifierProvider<AdminEmployeeProvider>(
          create: (ctx) => AdminEmployeeProvider(ctx.read<AdminApiService>()),
        ),
        ChangeNotifierProvider<AdminAttendanceProvider>(
          create: (ctx) => AdminAttendanceProvider(ctx.read<AdminApiService>()),
        ),
        ChangeNotifierProvider<AdminLeaveProvider>(
          create: (ctx) => AdminLeaveProvider(ctx.read<AdminApiService>()),
        ),
        ChangeNotifierProvider<AdminSalaryProvider>(
          create: (ctx) => AdminSalaryProvider(ctx.read<AdminApiService>()),
        ),
        ChangeNotifierProvider<AdminSettingsProvider>(
          create: (ctx) => AdminSettingsProvider(ctx.read<AdminApiService>()),
        ),
        ChangeNotifierProvider<AdminNotificationsProvider>(
          create: (ctx) => AdminNotificationsProvider(ctx.read<AdminApiService>()),
        ),
        Provider<AdminReviewService>(
          create: (ctx) => AdminReviewService(ctx.read<ApiService>()),
        ),
        ChangeNotifierProvider<AdminReviewProvider>(
          create: (ctx) => AdminReviewProvider(ctx.read<AdminReviewService>()),
        ),
      ...
      child: MaterialApp(
        ...
        routes: {
          ...
          '/admin-home': (context) => const AdminRouteGuard(child: AdminDashboard()),
...
/// Defense-in-depth: even if a button somewhere navigates to '/admin-home'
/// directly (bypassing the login form's own validation), this guard blocks
/// access unless the user actually holds a valid auth token, and bounces
/// them back to the admin login screen otherwise.
...
class _AdminRouteGuardState extends State<AdminRouteGuard> {
  ...
  Future<void> _check() async {
    ...
    // Re-validate against secure storage directly (not just in-memory
    // provider state), in case this route was reached via a fresh
    // navigation stack (e.g. deep link) where AuthProvider hasn't been
    // initialized yet.
    bool loggedIn = authProvider.isLoggedIn;
    if (!loggedIn) {
      loggedIn = await authProvider.apiService.isLoggedIn();
    }
    if (!mounted) return;
    setState(() => _authorized = loggedIn);
    if (!loggedIn) {
